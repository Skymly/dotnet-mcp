...
MCP 客户端以 **stdio** 连接该进程。默认受信根为进程当前工作目录。
...
> 完整用户文档将随发布完善；以下为产品安全基线摘要。

1. **受信根**：服务器只能读受信根内的路径。多仓库场景请显式配置额外根。
...
4. **日志（占位）**：计划仅记录操作与路径元数据、不记录源码正文；默认本地、可关闭；无外部遥测。当前骨架仅有进程控制台日志，尚未实现独立审计开关。
