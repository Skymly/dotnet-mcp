...
工具面由快照/守护测试约束：读工具 + 显式 rename preview/apply + Diagnostic fix。禁止通用写文件、补丁、命令或网络类工具。领域词汇见根目录 [`CONTEXT.md`](CONTEXT.md)。
...
3. **默认只读 + 受限 Workspace Edit**：读工具之外只有 rename 与 Diagnostic fix 的 preview/apply。不提供通用写文件、任意命令执行或网络请求类工具（有快照/守护测试约束）。
